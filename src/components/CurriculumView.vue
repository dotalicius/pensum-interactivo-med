<template>
  <div class="curriculum-view">
    <h2 class="text-2xl font-bold text-black mb-6">
      Plan de Estudios - {{ store.currentCareer.name }}
    </h2>

    <!-- Filtros -->
    <div class="mb-6 bg-white p-4 rounded-lg shadow-sm border">
      <h3 class="text-lg font-semibold text-black mb-3">Filtros</h3>
      <div class="flex flex-wrap gap-4">
        <div>
          <label class="block text-sm font-medium text-black mb-1">Año</label>
          <select
            v-model="selectedYear"
            class="border border-gray-300 rounded-md px-3 py-2 text-sm text-black focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
        ...
        <div>
          <label class="block text-sm font-medium text-black mb-1">Modalidad</label>
          <select
        ...
        <div>
          <label class="block text-sm font-medium text-black mb-1">Régimen</label>
          <select
        ...
        <div class="flex items-end">
          <button
            @click="clearFilters"
            class="bg-gray-100 hover:bg-gray-200 text-black px-4 py-2 rounded-md text-sm transition-colors duration-200 flex items-center"
          >
    ...
    <div v-if="groupedSubjects.length === 0" class="text-center py-12">
      ...
      <p class="text-black text-lg">No se encontraron materias con los filtros aplicados</p>
      <button
        @click="clearFilters"
    ...
    <div v-else class="space-y-8">
      <div v-for="yearGroup in groupedSubjects" :key="yearGroup.year" class="year-group">
        <div class="flex items-center mb-4">
          <div :class="getYearColorClass(yearGroup.year)" class="w-1 h-8 rounded-full mr-4"></div>
          <h3 class="text-xl font-semibold text-black">
            {{ yearGroup.year }}° Año
            <span class="text-sm font-normal text-black ml-2">
    ...
    <div class="mt-8 bg-gray-50 p-4 rounded-lg">
      <h4 class="font-semibold text-black mb-3">Leyenda de Estados</h4>
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm text-black">
...
const groupedSubjects = computed(() => {
  const grouped = filteredSubjects.value.reduce(
    (acc, subject) => {
      if (!acc[subject.year]) {
        acc[subject.year] = []
      }
...
const pendingCount = computed(
  () =>
    ...
    filteredSubjects.value.filter((s) => s.status === 'pending' && s.modality !== 'optional')
      .length,
)
